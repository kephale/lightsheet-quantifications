...
def process_imgdir( directory, close_after ) :
    ...
    all_2d_skeleton_edt_histograms['name'] += [ directory.split( '/' )[-1] + '_' + str(cube_idx) ]
    ...
    if not no_params_set:
        ...
        z_unit_scale = uniform.getCalibration().getY(1)
        y_unit_scale = uniform.getCalibration().getZ(1)
        ...
        # 2D measurement
        edt.close()
        voxelization_2d_skeleton = IJ.openImage( cube_basename + 'voxelization_maxproj.tif' )
        time.sleep( 5 )
        voxelization_2d_skeleton.show()
        IJ.run( voxelization_2d_skeleton, "Skeletonize (2D/3D)", "")
        time.sleep( 10 )

        IJ.run(voxelization_2d_skeleton, "32-bit", "")
        #time.sleep( 15 )
        IJ.run(voxelization_2d_skeleton, "Calculator Plus", 'i1=' + str(voxelization_2d_skeleton.getTitle()) + ' i2=' + str(voxelization_2d_skeleton.getTitle()) + ' operation=[Scale: i2 = i1 x k1 + k2] k1=0.003921568627 k2=0' )

        voxelization_2d_edt = IJ.openImage( cube_basename + 'voxelization_maxproj.tif' )
        time.sleep( 5 )
        IJ.run( voxelization_2d_edt, "Exact Euclidean Distance Transform (3D)", "" )
        time.sleep( 5 )
        voxelization_2d_edt = WindowManager.getImage( 'EDT' )

        ic = ImageCalculator()
        voxelization_2d_skeleton_edt = ic.run("Multiply 32-bit", voxelization_2d_edt, voxelization_2d_skeleton)
        IJ.run( voxelization_2d_edt, 'Fire', '' )

        # Get the histogram data in an array
        hist_nBins = int( ( hist_max - hist_min ) / hist_step )
        #skeleton_edt_stats = edt.getStatistics( Measurements.MEDIAN, hist_nBins, hist_min, hist_max )
        skeleton_edt_stats = StackStatistics( voxelization_2d_edt, hist_nBins, hist_min, hist_max )
        hist_data = skeleton_edt_stats.getHistogram()
        hist_binLabels = [ ( hist_min + hist_step * el ) for el in range( hist_nBins ) ]

        IJ.run( skeleton_edt, 'Histogram', 'bins=' + str(hist_nBins) + ' x_min=' + str(hist_min) + ' x_max=' + str(hist_max) + ' y_max=Auto stack' )
        skeleton_edt_histogram = WindowManager.getImage( 'Histogram of EDT' )
        IJ.saveAsTiff( skeleton_edt_histogram, cube_basename + 'skeleton_edt_2d_histogram.tif' )

        unit_scale = uniform.getCalibration().getX(1)

        f = open( cube_basename + 'skeleton_edt_2d_histogram.csv', 'w' )
        for k in range( len( hist_data ) ):
            f.write( str(hist_binLabels[k]) + '\t' + str(hist_data[k] * unit_scale) + '\n' )
            all_2d_skeleton_edt_histograms[hist_binLabels[k]] += [ hist_data[k] * unit_scale ]
        f.close()
...
parent_directory = '/Volumes/Amnes/Claudia/For_diameter_measurement/Diameter_measurement_confocal/'

dirlist = [ parent_directory + 'Near_optic_nerve_A_conf20x_crop0', \
            parent_directory + 'Near_optic_nerve_C2_conf20x_crop0', \
            parent_directory + 'Near_optic_nerve_D_conf20x_crop0', \
            parent_directory + 'Near_optic_nerve_E_conf20x_crop0' ]
